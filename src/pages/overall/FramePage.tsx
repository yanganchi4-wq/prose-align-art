...
const PositionCard: React.FC<PositionCardProps> = ({ 
  ...
  hint = "位置信息（绝对 + 相对）",
  namePrefix,
  refPlaceholder = "相对基准零部件",
  descPlaceholder = "相对关系描述（可选）"
}) => (
  <div className="rounded-[14px] bg-white/86 border border-foreground/14 p-3 col-span-full">
    <div className="flex items-center justify-between gap-3 mb-3">
      ...
      <span className="text-xs font-extrabold text-foreground/55">{hint}</span>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div className="rounded-xl bg-white/92 border border-foreground/14 p-3">
        ...
        <input className="w-full h-10 rounded-xl border border-foreground/14 bg-white/96 px-3 text-sm font-bold outline-none focus:border-accent/55 focus:ring-2 focus:ring-accent/16" placeholder="例如：mm，基于框架原点/ISO角件" />
      </div>
      <div className="rounded-xl bg-white/92 border border-foreground/14 p-3">
      ...
      <div className="relative rounded-[18px] bg-white/86 border border-foreground/10 shadow-card overflow-hidden p-6 z-10">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-black text-foreground/92 tracking-wide">总体设计 · 框架</h2>
            <span className="text-xs font-extrabold text-foreground/55">参数录入</span>
          </div>

          <form className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormInput label="框架类型" placeholder="例如：ISO 框架 / 加强型框架 …" />
              <FormInput label="框架尺寸" placeholder="例如：L×W×H 或 20ft/40ft" />
            </div>

            <FormInput label="框架材质" placeholder="例如：Q345B / Q355 / 不锈钢 …" />

            <PositionCard 
              title="底侧梁" 
              namePrefix="bottomSideBeam"
              refPlaceholder="相对基准零部件（例如：罐体中心线/端梁/角件）"
              descPlaceholder="相对关系描述（可选）：如对齐/间距/对称等…"
            />

            <PositionCard 
              title="顶侧梁" 
              namePrefix="topSideBeam"
              refPlaceholder="相对基准零部件（例如：罐体中心线/端梁/角件）"
              descPlaceholder="相对关系描述（可选）：如对齐/间距/对称等…"
            />
            ...
            <PositionCard 
              ...
              refPlaceholder="相对基准零部件（例如：底侧梁/端梁/罐体支座）"
              descPlaceholder="相对关系描述（可选）：如接地引出方向/避让要求等…"
            />

            <PositionCard 
              title="梯子" 
              namePrefix="ladder"
              refPlaceholder="相对基准零部件（例如：步道/平台/端梁）"
              descPlaceholder="相对关系描述（可选）：如与步道对齐/安装面/检修通行等…"
            />

            <PositionCard 
              title="步道" 
              namePrefix="walkway"
              refPlaceholder="相对基准零部件（例如：罐顶/梯子/阀箱区域）"
              descPlaceholder="相对关系描述（可选）：如检修区域覆盖/避让阀组件/防滑要求等…"
            />
